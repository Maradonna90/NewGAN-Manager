...
class Test_XML_Parser(unittest.TestCase):
    def test_parse_xml(self):
        test_xml = XML_Parser.parse_xml(None, self.paths.app+"test/test.xml")
...
class Test_RTF_Parser(unittest.TestCase):
    ...
    def test_parse_rtf(self):
        test_simple = RTF_Parser.parse_rtf(None, self.paths.app+"test/test_simple.rtf")
    ...
    def test_parse_rtf_with_short_UIDs(self):
        test_simple_UID = RTF_Parser.parse_rtf(None, self.paths.app+"test/test_simple_UID.rtf")
...
class Test_Config_Manager(unittest.TestCase):
    ...
    def test_get_latest_prf(self):
        latest_prf = Config_Manager().get_latest_prf(self.paths.app+"test/simple_cfg.json")
...
class Test_Xml_Writing(unittest.TestCase):
    def setUp(self):
        ...
        self.pm.prf_cfg["img_dir"] = self.paths.app+"test/"
        self.data = [
...
class Test_Profile_Manager(unittest.TestCase):
    ...
    def test_create_profile(self):
        ...
        cfg = Config_Manager().load_config(self.paths.app+".user/cfg.json")
        self.assertFalse(cfg["Profile"]["test"])
        self.assertTrue(os.path.isfile(self.paths.app+".user/test.json"))
        self.assertTrue(os.path.isfile(self.paths.app+".user/test.xml"))
        prf_cfg = Config_Manager().load_config(self.paths.app+".user/test.json")
    ...
    def test_swap_xml(self):
        self.pm.swap_xml("test", "No Profile", self.paths.app+"test/", self.paths.app+"test/")
        with open(self.paths.app+".user/test.xml", "r") as test_xml:
            self.assertEqual(test_xml.read(), "OUTSIDE")
        with open(self.paths.app+"test/config.xml", "r") as config_xml:
    ...
    def test_switching_profiles_with_invalid_path(self):
        self.pm.swap_xml("test", "No Profile", self.paths.app+"invalid/", self.paths.app+"test/")
        self.pm.swap_xml("No Profile", "test", self.paths.app+"test/", self.paths.app+"invalid/")
